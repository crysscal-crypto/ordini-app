...
import { ClipboardList, Users, Package, BarChart2 } from 'lucide-react'
...
export default function NavBar() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200">
      <div className="flex max-w-xl mx-auto gap-2 p-2">
        {tabs.map(({ to, icon: Icon, label, bg, activeBg }) => (
          <NavLink key={to} to={to} className="flex-1">
            {({ isActive }) => (
              <div className={`flex flex-col items-center justify-center py-3 gap-1 rounded-2xl transition-all duration-150 active:scale-95 ${isActive ? activeBg : bg}`}>
